...
expt_name = 'rbf12_diag_double_picnn_eigsig_bugfix2_flipflop'
rbf = 12
lr = 1e-1
lr_decay = False
lr_cl = 1e-3
...
if random_init:
    winit = torch.empty(rbf, 1).normal_(mean = 0, std = 1/np.sqrt(rbf))
else:
...
for run in range(10):
    ...
    for fold in range(10):
        ...
        for epoch in range(max_epoch):
            ...
            switch_time = (epoch % wavelet_opt == 0)
            if switch_time :
                if (epoch + wavelet_opt) % (2*wavelet_opt) == 0:
                    filt_opt = False
                elif epoch % 2*wavelet_opt == 0:
                    filt_opt = True
