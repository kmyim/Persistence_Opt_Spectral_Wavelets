...
class ChebyshevWavelets(nn.Module):
    ...
    def forward(self, mb):
        ...
        for i in range(L):
            ...
            datum['simplex_tree'] = utils.filtration_update(datum['simplex_tree'], f.detach().numpy())
            ...
            for k in range(3):
                ...
                if len(b) > self.max_num_intervals:
                    ...
                    truncation = torch.topk(p, self.max_num_intervals, largest=True, sorted=False).indices
                    b = b[truncation]
                    d = d[truncation]
