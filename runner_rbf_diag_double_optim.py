...
from models import ModelPIRBFDouble, EMA
...
for i in range(len(graph_list)):
    ...
    if diameter > 1:
        ...
        if type(Alist) == type(None):
            Alist = A
            hkslist = hks
            hks_anotherlist = hks_another
        else:
            Alist = np.append(Alist, A, axis = 0)
            hkslist = np.append(hkslist, hks)
            hks_anotherlist = np.append(hks_anotherlist, hks_another)
...
if random_init:
    winit = torch.empty(new_bfs, 2).normal_(mean = 0, std = 1/np.sqrt(new_bfs))
else:
    init_fs = np.vstack((hkslist, hks_anotherlist)).T
    winit = np.matmul(u.T, init_fs) # num params x 2
    reconstruct_delta = np.abs(np.matmul(u, winit)- init_fs)
    error_max = np.argmax(reconstruct_delta[:,0])
    print('max error 1', reconstruct_delta[error_max, 0], init_fs[error_max, 0])
    error_max = np.argmax(reconstruct_delta[:,1])
    print('max error 1', reconstruct_delta[error_max, 1], init_fs[error_max, 1])
    print('winit shape', winit.shape)
    winit = torch.tensor(winit).float()

mx1, mn1 = -np.inf, np.inf
mx2, mn2 = -np.inf, np.inf
...
for i in range(len(graph_list)):
    ...
    if dat['diameter'] > 1:
        ...
        cnter+= len(G)
        hks = torch.matmul(dat['secondary_gram'], winit).float().detach()

        mx1 = max(float(torch.max(hks[:,0])), mx1)
        mn1 = min(float(torch.min(hks[:,0])), mn1)
        mx2 = max(float(torch.max(hks[:,1])), mx2)
        mn2 = min(float(torch.min(hks[:,1])), mn2)

print('min, max', mn1,  mx1, mn2, mx2)
del u, s, vh
...
for run in range(10):
    ...
    for fold in range(10):
        ...
        torch.set_rng_state(rng_state) #fix init state
        torch.manual_seed(0)
        pht = ModelPIRBFDouble(rbf = new_bfs, resolution = resolution, lims = [[mn1, mx1], [mn2, mx2]], weightinit = winit, extra_feat_len = xtra_feat_length)
        ...
        eval_model = ModelPIRBFDouble(rbf = new_bfs, resolution = resolution, lims = [[mn1, mx1], [mn2, mx2]], weightinit = winit, extra_feat_len = xtra_feat_length)
